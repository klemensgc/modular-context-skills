...
1. **Skills registry** — `modular-context-skills/registry.json` — wszystkie skille z `requires[]` (liczba = długość `skills[]`, nie hardkoduj)
...
type: log
data: {YYYY-MM-DD}
agent: /skills-audit v{skill version}
---
```

`agent:` niesie wersjonowaną self-referencję (dawne `audited-by:`) — to wymagane pole typu `log`
(`_schemas/log.yaml`), więc nie dubluj go osobnym polem ani komentarzem HTML.
...
**Frontmatter required** (`_workspace/**` → `type: log`, `_schemas/log.yaml`): `title`, `type: log`,
`data` (dzisiejsza data), `agent` (`/skills-audit v{wersja}` — versioned self-reference).
Nie dopisuj `updated:` (stampuje pre-commit), `status:` ani `audited-by:` — pierwsze dwa nie należą
do typu `log`, trzecie jest reliktem 1.0. Weryfikacja: `python3 _claude/9-automation/schema_lint.py <plik>` → 0 FAIL
...
- **No plugin installed yet:** skip Step 2; report all registry skills as "Not installed — install plugin first"
